...
- **Audit (independent fresh-context review) scales with size/risk — it is a standard step, not an afterthought.** A separate agent or model with fresh context (e.g. Codex, or a different model) reviews the work; **for L (or shared-contracts / auth / data / R3+) an audit is required, not optional; for M invoke it when the diff is non-trivial; skip it for trivial S.** Two touchpoints: **(a) plan audit** before building (see step 2 — catch ordering/architecture flaws while they're cheap to fix); **(b) diff audit** before the merge gate (adversarial review of the change). Anchor on objective signals, run **under a wall-clock timeout with a self-validation fallback** (never silently block on a hung validator). If no second agent/model is available, do a fresh-context self-review and say so. Persist findings in `evidence/audit-*.md`; fold them back into the plan/diff before proceeding (an audit you don't act on is theatre). The human gates still decide — an audit informs them, it doesn't replace goal/merge approval.
