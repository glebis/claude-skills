---
version: alpha
name: "The Dalí Museum — Giraffes on Horseback Salad"
description: "Surrealist, cinematic landing/presentation brand: near-black cosmic base, coral fire as primary, cyan as cool counterweight, with orchid/acid/crimson dream accents."
colors:
  abyss-900: "#082224"
  accent: "#2fb9ca"
  acid-400: "#faff00"
  background: "#070613"
  coral-500: "#e4501b"
  crimson-500: "#c4151f"
  cyan-200: "#9adfe7"
  cyan-400: "#2fb9ca"
  danger: "#c4151f"
  gold-500: "#de8103"
  highlight: "#fbbbfe"
  ink-900: "#0b0e12"
  mist-100: "#f3f4f4"
  muted: "#5f5f60"
  orchid-300: "#fbbbfe"
  paper-50: "#fefefe"
  primary: "#e4501b"
  slate-500: "#5f5f60"
  slate-900: "#1a1b1d"
  surface: "#082224"
  teal-500: "#02708d"
  teal-700: "#023350"
  text: "#fefefe"
  text-ink: "#1a1b1d"
  void-950: "#070613"
  warning: "#faff00"
typography:
  body:
    fontFamily: "Inter"
    fontSize: "18px"
    fontWeight: "400"
    lineHeight: 1.6
  caption:
    fontFamily: "Inter"
    fontSize: "12px"
    fontWeight: "500"
    lineHeight: 1.4
    letterSpacing: "0.15em"
  display:
    fontFamily: "Playfair Display"
    fontSize: "72px"
    fontWeight: "500"
    lineHeight: 1.0
  heading:
    fontFamily: "Oswald"
    fontSize: "40px"
    fontWeight: "700"
    lineHeight: 1.05
rounded:
  md: "4px"
  sm: "2px"
spacing:
  2xl: "128px"
  lg: "32px"
  md: "16px"
  sm: "8px"
  xl: "64px"
  xs: "4px"
---

# The Dalí Museum — Giraffes on Horseback Salad

## Overview

Surrealist, cinematic landing/presentation brand: near-black cosmic base, coral fire as primary, cyan as cool counterweight, with orchid/acid/crimson dream accents.

## Colors

- **abyss-900** (`#082224`)
- **accent** (`#2fb9ca`)
- **acid-400** (`#faff00`)
- **background** (`#070613`)
- **coral-500** (`#e4501b`)
- **crimson-500** (`#c4151f`)
- **cyan-200** (`#9adfe7`)
- **cyan-400** (`#2fb9ca`)
- **danger** (`#c4151f`)
- **gold-500** (`#de8103`)
- **highlight** (`#fbbbfe`)
- **ink-900** (`#0b0e12`)
- **mist-100** (`#f3f4f4`)
- **muted** (`#5f5f60`)
- **orchid-300** (`#fbbbfe`)
- **paper-50** (`#fefefe`)
- **primary** (`#e4501b`)
- **slate-500** (`#5f5f60`)
- **slate-900** (`#1a1b1d`)
- **surface** (`#082224`)
- **teal-500** (`#02708d`)
- **teal-700** (`#023350`)
- **text** (`#fefefe`)
- **text-ink** (`#1a1b1d`)
- **void-950** (`#070613`)
- **warning** (`#faff00`)

## Typography

- **body** — Inter 18px/1.6 400
- **caption** — Inter 12px/1.4 500
- **display** — Playfair Display 72px/1.0 500
- **heading** — Oswald 40px/1.05 700

## Layout

Spacing scale: 2xl 128px, lg 32px, md 16px, sm 8px, xl 64px, xs 4px.

## Shapes

Corner radii: md 4px, sm 2px.
