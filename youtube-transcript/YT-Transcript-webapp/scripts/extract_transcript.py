...
Saves to ~/Downloads/yt-transcripts/ with YAML frontmatter.
"""
...
def validate_youtube_url(url: str) -> None:
    """Reject non-YouTube URLs to limit yt-dlp extractor attack surface."""
    parsed = urllib.parse.urlparse(url)
    if parsed.hostname not in ALLOWED_HOSTS:
        sys.exit(f"Error: Only YouTube URLs are supported, got: {parsed.hostname}")
...
def extract_subtitles(url, video_id):
    ...
    tmp_dir = tempfile.mkdtemp(prefix=f"yt_{video_id}_")
    output_template = f'{tmp_dir}/{video_id}'

    langs = os.environ.get("YT_TRANSCRIPT_LANGS", "en,ru").split(",")
    for lang in langs:
        for old_file in Path(tmp_dir).glob(f'{video_id}*.vtt'):
            old_file.unlink()
        ...
        subtitle_files = list(Path(tmp_dir).glob(f'{video_id}*.vtt'))
        if subtitle_files:
...
def main():
    ...
    vault_path = Path(os.environ.get("YT_TRANSCRIPT_OUTPUT_DIR", str(Path.home() / "Downloads" / "yt-transcripts")))
    if custom_filename:
